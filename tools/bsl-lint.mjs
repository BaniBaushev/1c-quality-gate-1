...
/** Присваивания, доказывающие ссылочность: результат этих выражений — ссылка и только она. */
const REF_SOURCES = [
  /\.\s*Ссылка\s*;?\s*$/iu,
  /\.\s*НайтиПо[А-Яа-яЁёA-Za-z]*\s*\(/iu,
  /\.\s*ПустаяСсылка\s*\(/iu,
  /\.\s*ПолучитьСсылку\s*\(/iu,
];
...
export function refVarsInRoutine(masked, routine, headerRefs) {
  ...
  while ((m = assign.exec(masked)) !== null) {
    ...
    const proven = REF_SOURCES.some((re) => re.test(m[2]));
