 ...
 * пропускает: `bsl-analyzer` 0.2.66 связывает голый локальный вызов с объявлением (иначе не
 * работал бы `MismatchedArgCount`), но об ОТКАЗЕ связывания молчит. Прямая проверка «имя не
