...
След показывает не только найденное: почему контур XML неприменим и что компилируемость тел
модулей осталась непроверенной. Недоступный анализатор и отсутствующий индекс кода фиксируются
такими же явными записями. Гейт снимается только по корректному следу — его разбирает
