...
Плагин Claude Code для контроля качества разработки на 1С:Предприятие. Работает с исходниками
в файлах: модули BSL и XML-выгрузки конфигураций и расширений. Блокирует завершение сессии,
пока изменённые файлы не проверены.

Контуров проверки четыре:

* `code`: диагностики статического анализатора, антипаттерны платформы, стандарты разработки 1С;
* `arch`: распределение ответственности, границы модулей, дублирование, переусложнение;
* `xml`: структура файлов метаданных, регистрация объектов в составе конфигурации, права ролей;
* `hygiene`: кодировка, BOM, управляющие символы, переводы строк.

> **Статус.** Все четыре контура работают, контур кода переведён на статический анализатор.
> Установка с GitHub и обновление проверены; прогон проверен на выгрузках типовой конфигурации
> и на боевых расширениях. Новая проверка принимается только с тестами, тесты проверяются
> мутациями.
...
| Отчёт «чисто» умалчивает о том, что часть проверок была недоступна | синтаксическая ошибка в теле процедуры проявляется при инициализации модуля в базе | без записи `not_verified` полностью чистый вердикт валидатор отклоняет |
...
Результат прогона состоит из двух частей.

**Отчёт для человека:** находки по severity (🔴 Critical, 🟠 Major, 🟡 Minor), у каждой ключ
локации `путь::Метод:строка`, правило (`#stdNNN`, код диагностики или идентификатор эвристики),
измеренное значение против порога и способ исправления.

**След для машины:** по строке на каждую выполненную и пропущенную проверку. Недоступный
анализатор, отсутствующий индекс кода и непроверенная компилируемость фиксируются явными
записями. След разбирает `evidence-validator.mjs`, поэтому вердикт проверяем независимо от
того, что утверждает отчёт.
...
**Кому адресован:** разработчику 1С, работающему в паре с Claude Code над выгрузкой
...
работоспособности: `/gate-status` отвечает «Гейт не взведён».
...
данных плагина. Python (валидаторы XML), индекс кода и справочник API платформы необязательны
и расширяют покрытие. Подробности в [docs/INSTALL.md](docs/INSTALL.md).
...
| `code` | [`bsl-code-review`](skills/bsl-code-review/SKILL.md) | с `C1` слой 1; на `C2` дополнительно слой 2; на `C3` те же слои плюс предложение слоя 3 | слой 1а: диагностики статического анализатора. Слой 1б: антипаттерны производительности и механики платформы, [антипаттерны кода языковой модели](skills/bsl-code-review/references/ai-antipatterns.md), стандарты под архетип, именование, сверка сигнатур API и существования общих модулей. Слой 2: ревью логики моделью | `tools/analyzer-run.mjs`, MCP `v8std`; при наличии индекс кода и справочник API платформы |
| `arch` | [`bsl-architecture-review`](skills/bsl-architecture-review/SKILL.md) | с `C2` уровни 1-2, на `C3` уровень 3; архетип поднимает уровень независимо от класса | десять признаков `ARCH-A1…A10`: модуль-комбайн, ветвление вместо полиморфизма, дубль-алгоритм, параллельная коллекция, перекладка переменных, инлайн-конструирование контракта, дублирующая валидация, инфраструктура в прикладном коде, экспорт без потребителей, переусложнение | [`signs-map.json`](skills/bsl-architecture-review/references/signs-map.md) с сигналом, порогом и контр-сигналом, MCP `v8std`, индекс кода |
| `xml` | [`xml-structure-review`](skills/xml-structure-review/SKILL.md) | по факту правки XML метаданных, независимо от объёма | сверка «диск ↔ состав» в обе стороны; валидация файлов объектов, форм, СКД, ролей, подсистем, макетов, командного интерфейса; права на объекты в ролях расширения | `tools/xml/orphan-check.mjs`, 10 валидаторов Python в `tools/xml/` |

Граница между `code` и `arch` проходит по радиусу исправления:

- исправление умещается внутри тела метода, значит это код;
- исправление выходит за пределы метода (выделение метода, перенос в другой модуль, новый
  экспорт, изменение «кто кого вызывает»), значит это архитектура.
...
Восемь строк показывают не только найденное, но и причину, по которой архитектурный контур не
запускался, почему контур XML неприменим и что компилируемость осталась непроверенной.

| Тип записи | Смысл | Обязательные поля |
|---|---|---|
| `scope` | профиль изменения, ровно одна запись за прогон | `volume`, `files`, `archetypes`, `driver`, `resolved` |
...
подтверждённого часового по каждому источнику, на который опирается вердикт `clean`, и запись
`not_verified` при полностью чистом вердикте. Грамматика и перечень причин приведены в
[evidence-format.md](skills/quality-gate/references/evidence-format.md).

Запись `not_verified` обязательна при чистом вердикте по следующей причине. Компилируемость тел
BSL не проверяют ни загрузка конфигурации из файлов, ни выгрузка, ни валидаторы XML: они
разбирают структуру, но тела не компилируют. Синтаксическая ошибка внутри процедуры проходит их
все и проявляется только при инициализации модуля в базе. Обнаруживает её проверка конфигурации
платформой или реальный запуск. Вердикт «чисто», умалчивающий об этом, закрывает вопрос
фальшивой зеленью.
...
**Отсутствие данных не превращается в «чисто».** Признаки `ARCH-A1`, `ARCH-A7` и `ARCH-A9`
...
```text
<корень проекта>/
├── .1c-quality-gate.json           необязательный конфиг: пороги осей, движок анализатора, часовой
└── .claude/.state/
    ├── qg-pending.json             взведённые гейты по сессиям и отметки проверенного (файл × слой)
    ├── qg-done.json                журнал снятий: класс, причина, охват
...
Все поля необязательны, незаданные принимают значения по умолчанию.
...
| Номер стандарта для часового по `v8std` | `sentinel.id`; закрепляется при установке, иначе неудачу запроса не отличить от исчезновения номера |
