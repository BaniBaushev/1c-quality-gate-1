...
Инструмент печатает готовые записи следа — по одной на правило. Он видит только литералы в
`.bsl`: запросы из XML (СКД, компоновщики формы) и собранные конкатенацией остаются
