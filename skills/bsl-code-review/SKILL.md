...
Не запускается автоматически: это дорого. Контур лишь **предлагает** его в отчёте для
крупных или критичных изменений.

При подтверждении — веер ревьюеров по измерениям (корректность, запросы, транзакции, события,
клиент-сервер, безопасность), затем по каждой находке независимые скептики, пытающиеся её
опровергнуть; проходит только то, что большинство не опровергло.
