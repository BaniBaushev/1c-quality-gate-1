...
Ищет псевдоним источника, совпадающий с именем колонки временной таблицы того же пакета
(`qg:QRY-ALIAS-SHADOWS-FIELD`). Обращение `Псевдоним.Поле` становится двояким, и платформа
отказывается выполнять запрос: «Неоднозначное поле».

Проверка вынесена в инструмент, потому что текст запроса — строковый литерал: его не
...
Разыменование коллидирующего имени — 🔴 (запрос не выполнится). Коллизия без обращения через
точку — 🟠: сейчас работает, но первое же добавленное поле подорвёт.

Инструмент печатает готовую запись следа. Он видит только литералы в `.bsl`: запросы из XML
(СКД, компоновщики формы) и собранные конкатенацией остаются непроверенными, и вердикт
«чисто» этого не отменяет.
