...
Для каждого вызова глобального метода, конструктора или метода типа сверь сигнатуру со
справочником платформы (MCP `1c-platform` либо аналог):
...
> **Вызовы справочника платформы — строго последовательно.** Параллельные вызовы этого MCP
> убивают сессию транспорта. Один вызов за ход, дождись результата. При `Connection closed`
> прекрати обращения к нему, отметь в «не проверено» и продолжай остальные проверки.
