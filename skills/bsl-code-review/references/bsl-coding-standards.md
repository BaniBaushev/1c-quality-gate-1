...
Всегда давать псевдонимы через `КАК`. Псевдоним таблицы должен быть информативным, **не должен совпадать** с именем поля — приводит к конструкции `Алиас.Алиас` и ошибке платформы.
