...
- [ ] Реквизиты ссылки читаются через `ОбщегоНазначения.ЗначениеРеквизитаОбъекта` / `ЗначенияРеквизитовОбъекта`, а не через точку: точка читает объект целиком (#std437, `qg:BSL-REF-DOT-ACCESS`, прогоняется `tools/bsl-lint.mjs`). Инструмент опознаёт ссылку по имени, оканчивающемуся на «Ссылка», — это нижняя граница проверки: ссылку, пришедшую параметром без подсказки в имени, ищи глазами.
