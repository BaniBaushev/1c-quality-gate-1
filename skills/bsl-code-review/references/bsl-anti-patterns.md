...
**Механизированная часть: `qg:BSL-REF-DOT-ACCESS` (`tools/bsl-lint.mjs`), 🔴.** Инструмент
опознаёт ссылку по конвенции именования — имя базы оканчивается на «Ссылка»
(`ЗаказКлиентаСсылка.Дата`, включая хвост цепочки `Структура.ДокументСсылка.Поле`). Это
**нижняя граница** проверки #std437, а не верхняя: ссылку, пришедшую параметром без подсказки
в имени, инструмент не распознаёт — разбор глазами он не отменяет.
