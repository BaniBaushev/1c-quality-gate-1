...
**Механизированная часть: `qg:BSL-REF-DOT-ACCESS` (`tools/bsl-lint.mjs`).** Ссылка опознаётся
по имени двумя способами:

- **конвенция «…Ссылка»** — `ЗаказКлиентаСсылка.Дата`, включая хвост цепочки
  `Структура.ДокументСсылка.Поле`. Severity 🔴, как у самого антипаттерна;
- **проектный словарь** `refDotAccess.refNames` — доменные имена, конвенции не подчиняющиеся;
  совпадение точное или по окончанию, «ё» и «е» не различаются. Severity 🟠: основание — чужой
  список, а не механика платформы. Ненастроенный словарь виден в следе как `config=default`.

Это **нижняя граница** проверки #std437, а не верхняя: ссылку, пришедшую параметром без
подсказки ни в имени, ни в словаре, инструмент не распознаёт — разбор глазами он не отменяет.
...
Более 4 уровней условий/циклов. Применять guard clauses (см. `bsl-refactoring.md`).
