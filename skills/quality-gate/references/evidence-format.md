...
Отметка должна быть **не старше последней правки файлов**: прогон, сделанный до правки,
описывает состояние, которого уже нет. Это то же правило, по которому гейт снимает отметки
проверенного при изменении файла.

Чего сверка не делает: она не доказывает, что инструмент смотрел именно эти файлы, и не
защищает от записи, дописанной в журнал вручную. В отличие от поля `config`, где истина
заново выводится с диска, независимого источника здесь нет. Это обнаружение молчания —
и сказано об этом прямо, чтобы никто не принял его за большее.
...
`not_in_analyzer_report` — изменённый файл движок не видел вовсе: вне корня конфигурации,
отсечён фильтром подсистем, не передан. Пока этой записи не было, такой файл давал пустой
результат, неотличимый от чистого. Число непроверенных файлов уходит в журнал прогонов, и
молчание о них блокирует снятие гейта.
...
строка. Лексические проверки (`qg:QRY-ALIAS-SHADOWS-FIELD`) снимают один класс дефектов, но
...
Последнее — не придирка: правка текста запроса прошла лексическую проверку, но «Поле не
