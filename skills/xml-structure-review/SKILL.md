...
```bash
node "${CLAUDE_PLUGIN_ROOT}/tools/xml/orphan-check.mjs" <каталог выгрузки>
```
...
Валидаторы принимают путь относительно корня проекта:

```bash
python "${CLAUDE_PLUGIN_ROOT}/tools/xml/meta-validate.py" -ObjectPath "<путь к Объект.xml>"
```
